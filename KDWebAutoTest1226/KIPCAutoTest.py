...
import time,os,logging,ConfigParser,threading,runstart,ctypes,telnetlib,sys
...
class GUI(Frame,API):
    ...
    #######定义开始按钮的后的一系列操作########################
    def  check_telnet(self):               #定义telnet及ssh，判断安全服务--SSH是否启用
        self.cf = ConfigParser.ConfigParser()
        self.cf.read(r'Config.ini')
        ...
        self.ssh_return = 2
        try:
            self.tn = telnetlib.Telnet(self.host,port=17230,timeout=10)
            self.tn.open(self.host,port=17230)
            self.tn.read_until("Username:",timeout=2)
            ...
            self.ssh_return = 1
            print msg
        print 'self.ssh_return',self.ssh_return
        time.sleep(1)
        if self.ssh_return==0: 
            self.check_out.set("SSH已开启，可以进行测试")
            self.ipc_func()
            print "testing  start !!!"
            time.sleep(10)
            self.all_enable()
            #time.sleep(1)
            #print "测试已完成，可以重新开始"
            #self.check_out.set("测试已完成，可以重新开始")
            return
        elif self.ssh_return==1: 
            self.check_out.set("SSH未开启，请先登录IPC进入“配置>系统>用户安全>安全服务”勾选启用SSH登录")
            self.all_enable()
            print "SSH未开启"
        else:
            pass
    ...
    def  check_ping(self):               #定义ping IPC的IP，判断配置网络是否通
        ...
        if self.ping_return: 
            self.ping_out= str('Ping IPC %s 网络失败，请检查IPC地址是否错误和网络是否连接'%self.host) #需要考虑如何显示在GUI上面
            self.all_enable()
            self.check_out.set(self.ping_out)
            return
        else: 
            self.ping_out=str('Ping IPC %s 网络无异常，已开始测试，注意不要关闭程序'%self.host) #需要考虑如何显示在GUI上面
            self.check_telnet()
            self.check_out.set(self.ping_out)
